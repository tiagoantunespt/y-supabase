...
var SupabaseProvider = class {
	...
	queueBroadcast(update) {
		...
		this.broadcastTimeout = setTimeout(() => {
			...
			const mergedUpdate = this.pendingUpdates.length === 1 ? this.pendingUpdates[0] : yjs.mergeUpdates(this.pendingUpdates);
			this.pendingUpdates = [];
	...
	handleRemoteUpdate(payload) {
		...
		try {
			...
			yjs.applyUpdate(this.doc, update, "remote");
	...
	*/
	sendStateVector() {
		if (!this.channel) return;
		const payload = {
			stateVector: encodeUpdate(yjs.encodeStateVector(this.doc)),
			user: { id: this.userId },
			timestamp: Date.now()
		};
		this.channel.send({
	...
	*/
	handleStateVector(payload) {
		...
		try {
			...
			const diff = yjs.encodeStateAsUpdate(this.doc, remoteStateVector);
	...
	*/
	connect() {
		...
		this.channel.on("broadcast", { event: STATE_VECTOR_EVENT }, (data) => {
			...
			} else if (status === _supabase_supabase_js.REALTIME_SUBSCRIBE_STATES.TIMED_OUT) {
				this.setStatus("disconnected");
				this.emit("error", /* @__PURE__ */ new Error("Connection timed out"));
				this.emit("disconnect", this);
				this.scheduleReconnect();
			} else if (status === _supabase_supabase_js.REALTIME_SUBSCRIBE_STATES.CLOSED) {
				this.setStatus("disconnected");
				this.emit("disconnect", this);
				this.scheduleReconnect();
			}
